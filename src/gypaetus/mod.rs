...
use query::Query;
use schema::{DocID, Document, Schema, Value, Vector, VectorType};
...
impl IndexBase {
    ...
    fn new(schema: Schema, config: IndexConfig) -> GyResult<IndexBase> {
        ...
        Ok(Self {
            ...
            schema: schema,
            rw_lock: Mutex::new(()),
            wal: Arc::new(Wal::new(&config.disk_path, config.fsize, config.io_type)?),
            doc_offset: Vec::new(),
        })
    }
    ...
    fn inner_add(&self, doc: &Document) -> GyResult<()> {
        for field in doc.field_values.iter() {
            ...
            fw.add(*self.doc_id.read()?, field.value())?;
        }
        Ok(())
    }

    pub fn write_wal(&self) -> GyResult<()> {
        // let wal = self.wal;
